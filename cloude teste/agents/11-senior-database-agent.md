...
- Manter `ESTRUTURA_BANCO_DADOS.md` alinhado ao schema real em Rust.
- Versionar evolucoes de schema com migracoes idempotentes e testes de upgrade.
- Auditar performance de consultas principais com indices e fluxo de listagem de biblioteca.
- Garantir atomicidade em syncs e operacoes que escrevem em mais de uma tabela.
...
1. Mapear os contratos atuais de `LibraryEntry`/DTOs Tauri para tabelas SQLite.
2. Definir o schema minimo para persistir jogos manuais da Fase 2.
3. Definir como dados mockados em `aplicativo/src/data/mockLibrary.js` viram seed/fallback sem duplicar dados reais.
...
- Plano de testes de migracao, rollback logico e performance de queries.
