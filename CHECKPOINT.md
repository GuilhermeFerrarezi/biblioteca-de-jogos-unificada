...
Atualizacao em 2026-05-26: foi restaurado o gate Rust do backend Tauri no Windows apos diagnostico de `STATUS_ENTRYPOINT_NOT_FOUND (0xc0000139)` no binario unitario `app_lib-*.exe`. A falha era de carregamento nativo: o binario de teste puxava superficies Tauri/Wry/Tao que importavam `TaskDialogIndirect` de `comctl32.dll`, mas sem manifesto Common Controls v6 o Windows resolvia a DLL antiga e abortava antes de executar a suite. Para preservar o gate, `run()`, `bootstrap_library`, o modulo de comandos Tauri e os fluxos Steam OpenID/Xbox Live dependentes de `AppHandle`, eventos ou webview foram isolados do build unitario por `cfg(test)`, sem alterar o runtime normal do app.

Atualizacao em 2026-05-26: a cobertura command-adjacent foi recuperada sem reintroduzir o runtime nativo Tauri/Wry/Tao no binario unitario. A logica de preflight da sincronizacao Steam por conta foi extraida para `command_logic`, cobrindo resolucao de SteamID64/API key, mapeamento de erros do AuthVault e guarda contra sincronizacao concorrente. Os wrappers `#[tauri::command]` continuam finos e nao sao testados diretamente quando isso exigiria carregar superficies nativas. Validacoes: `cargo test` com `CARGO_TARGET_DIR` local passou com 141 testes, `cargo fmt -- --check` passou e `cargo check` passou; `dumpbin /imports` confirmou ausencia de `comctl32.dll`/`TaskDialogIndirect` no binario unitario filtrado. O proximo ciclo de roadmap permanece achievements, nao Epic.

Atualizacao em 2026-05-26: o corte de Steam achievements best-effort foi implementado mantendo achievements como enriquecimento complementar de background. A logica testavel agora classifica falhas de enrichment fora dos wrappers Tauri: rate limit para a rodada atual com erro sanitizado, erros recuperaveis de provider registram retry/backoff e falhas de rede permanecem transientes. Respostas privadas/sem dados de player achievements da Steam Web API viram cache vazio esperado, sem gravar payload bruto ou segredo, e o cache de progresso segue escopado por `steam_id64 + app_id` para preservar dados de outras contas no mesmo jogo. Validacao inicial: `cargo test` passou com 146 testes. Xbox achievements continuam em hold por compliance; Epic nao foi iniciado neste ciclo.

Atualizacao em 2026-05-26: o fluxo Steam achievements foi fechado ate a UI. `LibraryEntry` agora pode carregar `game.achievements` a partir do cache SQLite Steam, combinando schema e progresso do jogador para entregar totais, percentual e lista detalhada sem expor segredo ou payload bruto. A biblioteca mostra resumo de conquistas nos cards/lista, o painel de detalhes mostra uma secao compacta com progresso/lista, e as ordenacoes `Conquistas: maior`/`Conquistas: menor` usam progresso real mantendo jogos sem dados no fim. Achievements hidden bloqueadas ficam ocultas por padrao como `Conquista secreta` e podem ser reveladas manualmente; hidden ja desbloqueadas aparecem normalmente. O seletor de ordenacao foi ajustado para contraste no tema escuro. Xbox achievements continuam em hold por compliance; Epic nao foi iniciado neste ciclo.

Atualizacao em 2026-05-26: a UI de Steam achievements foi refinada para escalar melhor. O painel de detalhes agora mostra apenas um preview compacto com progresso, barra, alguns itens alcancados/pendentes e contador `+N`, enquanto a acao `Ver todas` abre um modal com busca e lista completa rolavel. O frontend confirmou que o backend/DTO nao trunca `GameAchievementsDto.items`; o corte de 24 itens estava na UI antiga. A lista completa ordena alcancadas primeiro, depois bloqueadas, empurrando hidden bloqueadas para depois das nao-hidden e usando texto visivel para ordem alfabetica e busca. Hidden bloqueadas nao reveladas continuam sem vazar nome/descricao real; depois da revelacao manual passam a aparecer e ser buscaveis pelo conteudo original. Hidden ja desbloqueadas aparecem normalmente com indicador discreto `(secreta)`. Percentual global de jogadores ficou fora deste ciclo; Xbox achievements continuam em hold por compliance; Epic nao foi iniciado.
